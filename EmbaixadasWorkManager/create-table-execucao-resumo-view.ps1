# Script para criar tabela ExecucaoResumoView no DynamoDB
# Esta tabela armazena a última execução de cada empresa para consultas rápidas

param(
    [string]$TableName = "ExecucaoResumoView",
    [string]$Region = "sa-east-1",
    [string]$ProfileName = "default"
)

Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "Criando Tabela ExecucaoResumoView" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
aws dynamodb create-table `
    ...
    --attribute-definitions `
        AttributeName=PK_VIEW,AttributeType=S `
        AttributeName=SK_VIEW,AttributeType=S `
    --key-schema `
        AttributeName=PK_VIEW,KeyType=HASH `
        AttributeName=SK_VIEW,KeyType=RANGE `
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  PK: VIEW#LAST_EXEC (fixo)" -ForegroundColor White
    Write-Host "  SK: EMP#<Sigla> (ex: EMP#MA)" -ForegroundColor White
    Write-Host ""
    Write-Host "Atributos:" -ForegroundColor Cyan
    Write-Host "  - ExecucaoId: ID da ultima execucao" -ForegroundColor White
